...
import { AdminAgentTable } from '../../../components/Admin/AdminAgentTable/AdminAgentTable'

export const AdminAddAgent: React.FC = () => {
  const [isAddPopupOpen, setIsAddPopupOpen] = useState<boolean>(false)

  return (
    <div className='w-100'>
      <AdminNavbar currentPage={'Delivery Agents'} />
      <div className='px-4 mt-4'>
        <div className='w-100 d-flex justify-content-between '>

          <RippleButton onClick={()=>setIsAddPopupOpen(!isAddPopupOpen)} className='bg-info border-info border-2 px-3 py-1 rouned-0 text-white rounded-0'>Add New Agent</RippleButton>
        </div>
        <AdminAgentTable />
      </div>
      ...
      {isAddPopupOpen &&
        <div className='position-fixed top-0 start-0 w-100 h-100 d-flex justify-content-center align-items-center bg-dark bg-opacity-50'>
          <div className='bg-black w-50 d-flex flex-column px-4 py-4 rounded'>
            <h4 className='text-white'>Add New Agent</h4>
